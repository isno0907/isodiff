...
from h_unet import UNet2DModel_H, DDPMPipeline_H
...
from utils import stereographic_proj, inv_stereographic_proj, riemmanian_metric, isometry_loss_t, isometry_loss_h, pl_regularizer, ExponentialMovingAverage
...
def parse_args():
    ...
    parser.add_argument("--hard", default=False, action="store_true", help="Apply hard path length regularizer")
...
def main(args):
    ...
    # print("before = ", type(model))
    model, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
        model, optimizer, train_dataloader, lr_scheduler
    )
    # print("after = ", type(model))
    # raise
                ...
                if args.prediction_type == "epsilon":
                    ...
                    if args.lambda_iso > 0 and iso_cond:
                        ...
                        if args.noG:
                            ...
                            iso_loss = args.lambda_iso * pl_regularizer(model, noisy_images, timesteps, args, accelerator.device, ema)
                        else:
