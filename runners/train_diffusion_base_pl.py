...
def get_loss_fn(config):
    ...
    def loss_fn(model, x, y=None):
        ...
        if config.train.pl_penalty:
            ...
            Ju = F.jvp(lambda x: model.eps(x, t, y=y), perturbed_data, u, create_graph=True)[1]
            JTJu = F.vjp(lambda x: model.eps(x, t, y=y), perturbed_data, Ju/100, create_graph=True)[1]
    ...
    while state['step'] < config.train.n_iters:
        ...
        for _, (train_x, train_y) in enumerate(dataset_loader):
            ...
            with autocast(enabled=config.train.autocast):
                loss, pl_penalty = loss_fn(diffusion_wrapper, x, y)
            ...
            if (state['step'] + 1) % config.train.log_freq == 0 and config.setup.global_rank == 0:
                logging.info('Loss: %.4f, PL: %.4f, step: %d' %
                             (loss, pl_penalty, state['step'] + 1))
            dist.barrier()
